 ...
 * @Data:$time$ $date$
 */
public class ExtentTestNGIReporterListenerBackup implements IReporter {
    //生成的路径以及文件名
    private static final String OUTPUT_FOLDER = "test-output/";
    private static final String FILE_NAME = "ExtenReport.html";

    private ExtentReports extent;
    private boolean createSuiteNode;

    @Override
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
        init();
        createSuiteNode = false;
        if(suites.size()>1){
            ...
            for (ISuiteResult r : result.values()) {
                ...
                if(resultNode != null){
                  /*  resultNode.assignCategory(suite.getName(),context.getName());
                   ...
                   // resultNode.getModel().setName("模块:"+ context.getName());
    ...
    private void init() {
        ...
        extent.setSystemInfo("Test executor","****");
        extent.setSystemInfo("Browser","Chown");
        ...
        if (tests.size() > 0) {
           ...
           /*     if(name.length()>0){
                ...
                if(extenttest==null){
                   ...
                   test = extenttest.createNode("方法名："+ result.getName()+"\n ["+result.getMethod().getDescription()+"]","参数："+ name).assignCategory(categories);
                }
